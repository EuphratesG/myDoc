...
28、Transformer 的并行体现在哪个地方？Decoder 端可以做并行化吗？
------------------------------------------------------
RNN的每一个样本内部必须一步一步地计算完成，而CNN和Self-Attention则可以完全并行计算。这里说的不能并行指的是每一个样本内部，不同样本彼此之间是可以进行并行计算的。这里的一个样本指的是一个句子。通常情况下，RNN 在每个时间步都会输出一个**隐藏状态**，当然最后的输出也是隐藏状态。初始h0为0。这样rnn只能一个个处理单词。  
而transformer则直接把句子中的单词embedding构成一个矩阵直接并行处理了。分编码器（全并行）和解码器（训练因为teacher forcing所以还是矩阵实现，但推理需要依赖之前的输出值所以是串行）  
